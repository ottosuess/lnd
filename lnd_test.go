...
func waitForChannelPendingForceClose(ctx context.Context,
	...
	err = lntest.WaitPredicate(func() bool {
		...
		for _, pendingClose := range pendingChanResp.PendingForceClosingChannels {
			if pendingClose.Channel.ChannelPoint == chanPointStr {
				forceClose = pendingClose
				found = true
				break
			}
		}
		if !found {
			predErr = fmt.Errorf("unable to find channel")
			return false
		}
...
func testOpenChannelAfterReorg(net *lntest.NetworkHarness, t *harnessTest) {
	...
	if err := tempMiner.SetUp(true, 50); err != nil {
		t.Fatalf("unable to set up mining node: %v", err)
	}
	defer tempMiner.TearDown()

	if err := tempMiner.Node.NotifyNewTransactions(false); err != nil {
		t.Fatalf("unable to request transaction notifications: %v", err)
	}

	// We start by connecting the new miner to our original miner,
	// such that it will sync to our original chain.
	err = net.Miner.Node.Node(btcjson.NConnect,
		tempMiner.P2PAddress(), &temp)
	if err != nil {
	...
	err = lntest.WaitPredicate(func() bool {
		...
		if tempMinerHeight != minerHeight+5 {
			predErr = fmt.Errorf("expected new miner(%d) to be 5 "+
				"blocks ahead of original miner(%d)",
				tempMinerHeight, minerHeight)
			return false
		}
		return true
	}, time.Second*15)
	...
	// Now we disconnect the original miner, and connect the the two miners
	// together. Since the temporary miner knows about a longer chain, both
	// miners should sync to that chain.
...
func testDisconnectingTargetPeer(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, timeout)
	err = waitForChannelPendingForceClose(ctxt, net.Alice, chanPoint)
	if err != nil {
		t.Fatalf("channel not pending force close: %v", err)
	}

	// Mine enough blocks for Alice to sweep his funds from the force
	// closed channel.
	...
	// Alice should now sweep the funds, clean up by mining the sweeping tx.
...
func assertPendingChannelNumHtlcs(
	forceClose *lnrpc.PendingChannelsResponse_ForceClosedChannel,
	expectedNumHtlcs int) error {

	if len(forceClose.PendingHtlcs) != expectedNumHtlcs {
		return fmt.Errorf("expected force closed channel to have %d pending "+
...
func assertNumForceClosedChannels(
	pendingChanResp *lnrpc.PendingChannelsResponse, expectedNumChans int) error {

	if len(pendingChanResp.PendingForceClosingChannels) != expectedNumChans {
		return fmt.Errorf("expected to find %d force closed channels, got %d",
			expectedNumChans,
...
func assertPendingHtlcStageAndMaturity(
	forceClose *lnrpc.PendingChannelsResponse_ForceClosedChannel,
...
func testChannelForceClosure(net *lntest.NetworkHarness, t *harnessTest) {
	...
	err = lntest.WaitPredicate(func() bool {
		...
		err = assertPendingChannelNumHtlcs(forceClose, numInvoices)
		if err != nil {
			predErr = err
			return false
		}

		err = assertPendingHtlcStageAndMaturity(forceClose, 1, htlcExpiryHeight, 1)
		if err != nil {
			predErr = err
			return false
		}
	...
	err = assertPendingHtlcStageAndMaturity(forceClose, 2,
	...
	err = assertNumForceClosedChannels(pendingChanResp, 0)
	if err != nil {
		t.Fatalf(err.Error())
	}
...
func testInvoiceRoutingHints(net *lntest.NetworkHarness, t *harnessTest) {
	...
	// Wait for the channel to be marked pending force close.
	ctxt, _ = context.WithTimeout(ctxb, timeout)
	err = waitForChannelPendingForceClose(ctxt, net.Alice, chanPointEve)
	if err != nil {
		t.Fatalf("channel not pending force close: %v", err)
	}
	...
	// Alice should now sweep her funds, clean up by mining the sweeping tx.
	txid, err := waitForTxInMempool(net.Miner.Node, 20*time.Second)
	if err != nil {
		t.Fatalf("unable to find sweeping tx in mempool: %v", err)
	}
...
out:
	...
	// Wait for the channel to be marked pending force close.
	ctxt, _ = context.WithTimeout(ctxb, timeout)
	err = waitForChannelPendingForceClose(ctxt, net.Bob, chanPointBob)
	if err != nil {
		t.Fatalf("channel not pending force close: %v", err)
	}

	// Mine enough blocks for Bob to sweep his funds from the force
	// closed channel.
	_, err = net.Miner.Node.Generate(defaultCSV)
	if err != nil {
		t.Fatalf("unable to generate blocks: %v", err)
	...
	// Bob should now sweep his funds, clean up by mining the sweeping tx.
	txid, err := waitForTxInMempool(net.Miner.Node, 20*time.Second)
	if err != nil {
		t.Fatalf("unable to find sweeping tx in mempool: %v", err)
	}
...
func testAsyncPayments(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, 3*time.Minute)
...
func testMultiHopRemoteForceCloseOnChainHtlcTimeout(net *lntest.NetworkHarness,
	...
	if err != nil {
		...
		// we'll fail.
		// TODO(halseth): use waitForChannelPendingForceClose to avoid
		// this.
		if _, err := net.Miner.Node.Generate(1); err != nil {
			t.Fatalf("unable to generate block: %v", err)
		}
...
func testMultiHopHtlcLocalChainClaim(net *lntest.NetworkHarness, t *harnessTest) {
	...
	// TODO: figure out why we need +1.
	if _, err := net.Miner.Node.Generate(carolSecondLevelCSV + 1); err != nil {
		t.Fatalf("unable to generate block: %v", err)
	}
...
func TestLightningNetworkDaemon(t *testing.T) {
	...
	args := []string{
		"--rejectnonstd",
		fmt.Sprintf("--connect=%v", chainBackend.P2PAddr()),
	}
