...
func testDataLossProtection(net *lntest.NetworkHarness, t *harnessTest) {
	...
	// Carol will be the up-to-date party.
	...
	// Send payments from Carol to Dave jusing 3 of Dave's payment hashes
	// generated above.
	ctxt, _ = context.WithTimeout(ctxb, timeout)
	...
	// TODO(halseth): make these wait predicate into assert methods.
	err = lntest.WaitPredicate(func() bool {
		pendingChansRequest := &lnrpc.PendingChannelsRequest{}
		pendingChanResp, err := carol.PendingChannels(ctxb,
			pendingChansRequest)
	...
	err = lntest.WaitPredicate(func() bool {
		pendingChansRequest := &lnrpc.PendingChannelsRequest{}
		pendingChanResp, err := dave.PendingChannels(ctxb,
			pendingChansRequest)
		if err != nil {
			predErr = fmt.Errorf("unable to query for pending "+
				"channels: %v", err)
	...
	daveSweep, err := waitForTxInMempool(net.Miner.Node, 5*time.Second)
	if err != nil {
