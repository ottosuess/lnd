...
func (b *mockArbitratorLog) FetchContractResolutions() (*ContractResolutions, error) {
	if b.failFetch {
		return nil, fmt.Errorf("intentional fetch failure")
	}
...
func assertStateTransitions(t *testing.T, newStates <-chan ArbitratorState,
	...
	for _, exp := range expectedStates {
		...
		case <-time.After(5 * time.Second):
			t.Fatalf("new state not received - line %v", line)
		}

		if state != exp {
			t.Fatalf("Line %v - expected new state %v, got %v",
				line, exp, state)
		}
	}
}
...
func TestChannelArbitratorPersistence(t *testing.T) {
	...
	// It should not advance further, however, as fetching resolutions failed.
	time.Sleep(100 * time.Millisecond)
