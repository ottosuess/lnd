...
// fetchLastChanUpdateByOutPoint fetches the latest polocy for our direction of
...
func (s *server) fetchLastChanUpdateByOutPoint(op wire.OutPoint) (
	*lnwire.ChannelUpdate, error) {
	...
	if bytes.Equal(ourPubKey, owner(edge2)) {
	...
	for {
		...
		case <-ticker.C:
			...
			for _, c := range channels {
				...
				// status has changed, set active status.
				st, ok := status[c.FundingOutpoint]
				if !ok || st.active != active {
					status[c.FundingOutpoint] =
						activeStatus{
							active: active,
							time:   time.Now(),
						}
